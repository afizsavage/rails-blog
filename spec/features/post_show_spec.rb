...
RSpec.describe 'post_show_route', type: :feature do
  describe 'Post' do
    ...
    it 'should display author name of posts' do
      expect(page).to have_content(Post.first.author.name)
    end
    it 'should display number of comments for post' do
      post = Post.first
      expect(post.comments_counter).to eq(post.comments.size)
    end
    it 'should display number of likes for post' do
      post = Post.first
      expect(post.likes_counter).to eq(post.likes.size)
    end
    it 'should display username of commentor' do
      comments = Comment.first
      expect(page).to have_content(comments.author.name)
    end
    it 'should display content of comment' do
      comments = Comment.first
