...
describe 'user index page', type: :feature do
  context 'accessing user index page' do
    before :each do
      ...
      @subject_user = User.create!(name: 'Juan', photo: 'img', bio: 'I am Juan', posts_counter: 0,
    ...
    it 'should show image of the user' do
      all('img').each do |i|
        expect(i[:src]).to eq('https://placekitten.com/200/200')
      end
    end

    it 'should show user biography' do
      expect(page).to have_content('I am Juan')
    end
